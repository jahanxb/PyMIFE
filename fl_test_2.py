...
scale = 100000  # Scale for precision
...
# Try dimension-wise decryption
y_int = np.zeros(m, dtype=int)
for dim in range(m):
    # Function vector to average the d-th dimension
    ...
    print(f"Dimension {dim} decrypted value: {val}")
    y_int[dim] = int(val) if isinstance(val, (int, float)) else val[0]
print(f"y_int shape: {y_int.shape}, values: {y_int}")
# Fallback if decryption fails
if not np.all(y_int):
    print("Warning: Dimension-wise decryption failed, using plaintext average for debugging")
    y_int = np.mean(client_w_int, axis=0).astype(int)
    print(f"Fallback y_int shape: {y_int.shape}, values: {y_int}")
