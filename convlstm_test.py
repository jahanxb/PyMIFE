...
class PeMSDataset(Dataset):
    ...
    def __getitem__(self, idx):
        x = self.data[idx:idx+self.seq_len]                   # [T,H,W,1]
...
if __name__=="__main__":
    # load & normalize
    data = np.load("pems07.npz")['data']  # [T,N,1]
    T,N,_ = data.shape
    h = int(math.floor(math.sqrt(N))); w = int(math.ceil(N/h))
    grid = (h,w)

    scaler = MinMaxScaler()
    flat   = data.reshape(-1,N)
    norm   = scaler.fit_transform(flat).reshape(T,N,1)
    grid   = reshape_to_grid(norm, grid)

    # dataset & splits
    ds      = PeMSDataset(grid, SEQ_LEN, PRED_LEN)
    train_n = int(0.8*len(ds)); rest = len(ds)-train_n
    train_ds, test_ds = random_split(ds, [train_n,rest])
    val_n   = int(0.1*train_n); train_n -= val_n
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        print(f"Epoch {epoch}: Train MSE={t_loss:.4f}  Val MSE={v_loss:.4f}")
